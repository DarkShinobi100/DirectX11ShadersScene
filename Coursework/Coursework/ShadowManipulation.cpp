...
ShadowManipulation::ShadowManipulation(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"ShadowManipulation_vs.cso", L"MasterShadowPixelShader.cso");
}


ShadowManipulation::~ShadowManipulation()
{
	if (sampleState)
	{
		sampleState->Release();
...
void ShadowManipulation::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
...
void ShadowManipulation::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, const XMMATRIX LightViewMatrix[13], const XMMATRIX LightprojectionMatrix[13],
	ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* Bumptexture, ID3D11ShaderResourceView* depthMap[13], Light* light[8], bool ToggleNormals,float time)
{
	...
	deviceContext->VSSetShaderResources(1, 1, &Bumptexture);
}
