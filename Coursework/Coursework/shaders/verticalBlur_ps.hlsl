...
float4 StrongBlur(InputType input, float4 colour, float4 DistanceValue)
{
    ...
    colour += (shaderTexture[0].Sample(SampleType, input.tex + float2(0.0f, texelSize * -4.0f)) * weight4) * (DistanceValue / DistanceValue.w);
    colour += (shaderTexture[0].Sample(SampleType, input.tex + float2(0.0f, texelSize * -3.0f)) * weight3) * (DistanceValue / DistanceValue.w);
    colour += (shaderTexture[0].Sample(SampleType, input.tex + float2(0.0f, texelSize * -2.0f)) * weight2) * (DistanceValue / DistanceValue.w);
    colour += (shaderTexture[0].Sample(SampleType, input.tex + float2(0.0f, texelSize * -1.0f)) * weight1) * (DistanceValue / DistanceValue.w);
    colour += (shaderTexture[0].Sample(SampleType, input.tex) * weight0) * (DistanceValue / DistanceValue.w);
    colour += (shaderTexture[0].Sample(SampleType, input.tex + float2(0.0f, texelSize * 1.0f)) * weight1) * (DistanceValue / DistanceValue.w);
    colour += (shaderTexture[0].Sample(SampleType, input.tex + float2(0.0f, texelSize * 2.0f)) * weight2) * (DistanceValue / DistanceValue.w);
    colour += (shaderTexture[0].Sample(SampleType, input.tex + float2(0.0f, texelSize * 3.0f)) * weight3) * (DistanceValue / DistanceValue.w);
...
float4 main(InputType input) : SV_TARGET
{
    ...
    if (DistanceValue.r >= shaderTexture[1].Sample(SampleType, float2(0.5, 0.5)).r)
    {
